...
def wm_inject_run(doc_path, loc_path, aim_count = 50 ):
    ...
    def filter_wmunit(wmunit_doc_list, count_doc, aim_count):
        ...
        for i in range(count_doc):
            if args.eval_dataset == "trec-covid":
                covid_count = 0
                contains_test = any("Covid-19" in s for s in wmunit_doc_list[i][0])
                if contains_test and covid_count <2:
                    covid_count +=1
                    continue
            if args.eval_dataset == "nfcorpus":
               
                contains_test = any("Alkaline Comet Assay" in s for s in wmunit_doc_list[i][0])
                if contains_test:
        
                    continue                
            rag_document = []
            if args.mutual_times!=0:
                rag_document = [j for j in range(len(wmunit_doc_list[i][1])) if wmunit_doc_list[i][2][j] == 1 and wmunit_doc_list[i][3][j] == 1]
            else:
                print("不用双check")
                rag_document = [j for j in range(len(wmunit_doc_list[i][1]))]
    ...
    for i in range(count_doc):
        ...
        rag_document = []
        rag_document = [j for j in range(len(aim_wmunit_list[i][1]))]
...
def wm_verify_run_k(verify_path, loc_path, doc_path, stat_path, filter_flag=False, verify_num=30,k=1):
    ...
    for wmunit_loc in rwmunit_loc_list:
        ...
        print(f"rag_document-{rag_document}")
        ...
        if filter_flag == True:
            ...
            # if len(rag_document) >200:
            #     checker.rag_document = rag_document[:20]
            # else:
            #     checker.rag_document = rag_document
    ...
    # delete_ids_path = '/home/sunmengjie/lpz/vectordb/ragwatermark/output_wh/delete_ids/deleteids.json'
    # save_json(ids_delete_list,delete_ids_path)
    # stat_result(verify_path, stat_path)
...
def parse_args():
    ...
    ### save path
    # parser.add_argument('--basepath', type=str, default='/home/sunmengjie/lpz/vectordb/ragwatermark/output_wh', help='save watermark_doc, inject_info, verify_info, stat_results')
...
if __name__ == '__main__':
    ...
    model_configs = "/data/sunmengjie/lpz/ragwm/model_configs"
