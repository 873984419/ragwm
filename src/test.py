...
#     elif data_name == 'nfcorpus':
#         corpus, queries, qrels = load_beir_datasets(data_name, 'test')
#         random.seed() 
#         random_query_ids = random.sample(list(queries.keys()), 323)
#         count = 0
#         for query_id in random_query_ids:
#             query_text = queries[query_id]
#             query_text_len = count_words(query_text)
#             if query_text_len == 1:
#                 count +=1
#                 print(f"{query_text_len}---{query_text}")
#         print(count)    
#     else:
#         corpus, queries, qrels = load_beir_datasets(data_name, 'test')
#     print(f"{data_name}_query_len:{len(queries)} {data_name}_corpus_len:{len(corpus)} {data_name}_qrels:{len(qrels)}")
...
def doc_llm_run_mutual(doc_path, mutual_times=0):
    ...
    for item, wmunit in enumerate(wmunit_list):
        ...
        # 使用红色字体在控制台输出结果
        print(f"{RED}{elapsed_time:.2f} seconds{RESET}")
...
if __name__ == '__main__':
    ...
    ## create vector db
    args.eval_dataset = "nfcorpus"
    args.eval_model_code = "contriever"
    args.score_function = "cosine"
